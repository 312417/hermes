...
scp -P "$PHONE_PORT" -i "$PHONE_SSH_KEY" \
  server.py hermes_store.py tool_registry.py model_provider.py natural_language.py telegram_bot.py supervisor.py runtime_check.py \
...
ssh -p "$PHONE_PORT" -i "$PHONE_SSH_KEY" "$REMOTE" \
  'mkdir -p ~/.termux/boot; install -m 700 start-hermes ~/.termux/boot/20-hermes; \
   ...
   for file in server.py hermes_store.py tool_registry.py model_provider.py natural_language.py telegram_bot.py supervisor.py runtime_check.py configure-secrets.sh README.md telegram.env.example; do \
